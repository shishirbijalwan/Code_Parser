...
int main(int argc, char* argv[])
{
  ...
  for(int i=1; i<argc; ++i)
  {
	...
	std::cout << "Tree Display start";
	configure.displayTree();
	//std::cout << "\n searching for node \n";
	//std::cout << "The pointer to the node we search is " << configure.searchNode("display");
	//std::cout << "\n Calculating the number of child nodes \n";
	//std::cout << "Number of child this node has is " << configure.calculateChildNodes("Another");
	/*std::cout << "\n collecting same type of nodes \n";
	std::vector<std::string> temp=configure.collectSingleType("function");
	std::cout << "\n size of vector returned is " << temp.size();
	for (int i = 0; i < temp.size(); i++) {
		std::cout << "\n Value " << i << " " << temp[i] << "\n";
	}*/

	std::cout << "\n Display Complexity \n";
	configure.DisplayMetrics("function");
